...
def load_train_data(page=1, page_size=10000):
    ...
    combined_en = fleurs_en_transformed.skip(skip_n).take(page_size)
    combined_ar = concatenate_datasets([fleurs_ar_transformed]).skip(skip_n).take(page_size)
    combined_tr = concatenate_datasets([fleurs_tr_transformed]).skip(skip_n).take(page_size)

    return {"en": combined_en, "ar": combined_ar, "tr": combined_tr}

def load_test_data(page=1, page_size=10000):
    # Define columns to keep in the final harmonized schema
    columns_to_keep = ['id', 'audio', 'transcription', 'language', 'gender']
    ...
    # Dataset 1: Fleurs
    fleurs_en = load_dataset("google/fleurs", "en_us", split="test", trust_remote_code=True, streaming=True, download_config=config)
    fleurs_ar = load_dataset("google/fleurs", "ar_eg", split="test", trust_remote_code=True, streaming=True, download_config=config)
    fleurs_tr = load_dataset("google/fleurs", "tr_tr", split="test", trust_remote_code=True, streaming=True, download_config=config)

    # Dataset 2: Voxpopuli (Only Supports English)
    voxpopuli_en = load_dataset("facebook/voxpopuli", "en", split="test", trust_remote_code=True, streaming=True, download_config=config)
